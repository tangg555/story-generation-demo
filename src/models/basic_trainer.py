...
class BasicTrainer(object):
    ...
    def _init_logger(self, args, model):
        ...
        """
        if (
                args.logger_name == "CSVLogger"
        ):
            self.logger = CSVLogger(save_dir=self.log_dir,
                                    name=f'{self.experiment_name}_CSVLogger')
        elif args.logger_name == "WandbLogger":
            from pytorch_lightning.loggers import WandbLogger
            self.logger = WandbLogger(name=f'{self.experiment_name}_WandLogger',
